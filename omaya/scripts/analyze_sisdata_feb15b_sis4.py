...
def analyze_all(lo_source='YIG'):
    ...
    for freq in freqs:
        ...
        #    basedir = 'may19_2021'
         ...
         #   basedir = 'may20_2021'
        basedir = 'feb15b_2022'
        fileglob = os.path.join(basedir, 'sis4_*_%sGHz_*_if1_hot.txt' % (freq))
        pat = os.path.join(basedir, 'sis4_\w+_%sGHz_(?P<power>\d+)mW_if1_hot.txt' % (freq))
        ...
        for filename in filenames:
            ...
            if match:
                ...
                power = ""
            ifcold_fname = filename.replace('if1_hot', 'if1_cold')
...
def make_ifpowers(dfout):
    ...
    for freq in freqs:
        ...
        fname = os.path.join(basedir, 'sis4_%s_%sGHz_%s_if1_hot.txt' % (basedir, freq, power.replace(' ', '')))
